...
def get_forward_details(datacut, select_pairs, select_tenors, cut_types):
    ...
    start_query = """Select min(date), max(date) from marketdata_fxforward
                where pair_id= :pair"""
    query = """Select date, extract(dow from date) as dow, pair_id, tenor, data_cut_id from marketdata_fxforward
    ...
    for pair in select_pairs:
        ...
        if (date_df.empty) | (start is None and stop is None):
            ...
            for tenor in select_tenors:
                ...
                if df.empty:
                    data_detail.append([None, None, pair, tenor, None])
                    ...
                    for cut in cut_types:
                        ...
                        if len(df_cut) == 0:
                            ...
                            data_detail.append([None, None, pair, tenor, cut])
                            continue
                        else:
                            # collect earliest record date for pair/tenor
                            data_detail.append([start, stop, pair, tenor, cut])
                                    ...
                                    if mini_window.index[0] in found_inx:
                                        ...
                                        if (dow_base == 5) & (dow_delta == -4):
                                            ...
                                            while beg_date != end_date:
                                                ...
                                                if count > 1:
                                                    data_missing.append([beg_date, int(beg_date.isoweekday()), pair, tenor, cut])
    ...
    detail_df = pd.DataFrame(data_detail, columns=['earliest_date', 'latest_date', 'pair_id', 'tenor', 'cut_type'])
    missing_df = pd.DataFrame(data_missing, columns=['date', 'dow', 'pair_id', 'tenor', 'cut_type'])
...
cut_types = [1,3]
datacut = get_datacut_df()
detail_df, missing_df = get_forward_details(datacut, all_pairs, all_tenors, cut_types)

df_missing_final = get_currency_info(missing_df, pair_map)
print(df_missing_final.head())
df_missing_final.to_csv("../storage/csv/missing data analysis/missing_data_fxforward.csv", index=False)

df_detail_final = get_currency_info(detail_df, pair_map)
print(df_detail_final.head())
df_detail_final.to_csv("../storage/csv/missing data analysis/details_data_fxforward.csv", index=False)
