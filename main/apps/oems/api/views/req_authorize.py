...
class ReqExecuteAuthorize(mixins.CreateModelMixin,
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                name='Authorize Reject',
                # description='some description'
                value={
                    'error_message': 'TICKET ALREADY AUTHORIZED',
                },
                response_only=True,
                status_codes=['406'],
            ),
    ...
    def create(self, request, *args, **kwargs):
        ...
        if ticket.action != 'execute' or ticket.auth_user:
            ...
            ticket.auth_user = request.user.id
            # DO NOT SAVE THIS ONE. LET THE OMS PROCESS IT

            resp = enqueue(f'api2oms_{settings.APP_ENVIRONMENT}', ticket.export(), uid=ticket.id, action=OMS_API_ACTIONS.AUTHORIZE, source='DJANGO_APP')
            print( 'ENQUEUED:', resp )
            # poll-id response
            data = { 'poll-id': resp }
