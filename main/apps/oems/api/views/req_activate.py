...
class ReqExecuteActivate(mixins.CreateModelMixin,
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                name='Activation Reject',
                # description='some description'
                value={
                    'error_message': 'TICKET IS NOT A DRAFT',
                },
                response_only=True,
                status_codes=['406'],
            ),
            OpenApiExample(
                name='Activate Accept',
                # description='some description'
                value={
    ...
    def create(self, request, *args, **kwargs):
        ...
        if ticket.action != 'execute' or ticket.internal_state not in INTERNAL_STATES.DRAFT:
            ...
            resp = enqueue(f'api2oms_{settings.APP_ENVIRONMENT}', ticket.export(), uid=ticket.id, action=OMS_API_ACTIONS.ACTIVATE, source='DJANGO_APP')
            print( 'ENQUEUED:', resp )
            # poll-id response
            data = { 'poll-id': resp }
