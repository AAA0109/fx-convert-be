...
class ReqExecuteCancel(mixins.CreateModelMixin,
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                value={
                    'error_message': 'TOO LATE TO CANCEL',
                },
                response_only=True,
                status_codes=['406'],
            ),
    ...
    def create(self, request, *args, **kwargs):
        ...
        if ticket.internal_state not in INTERNAL_STATES.CANCELLABLE_STATES:
            ...
            resp = enqueue(f'api2oms_{settings.APP_ENVIRONMENT}', ticket.export(), uid=ticket.id, action=OMS_API_ACTIONS.CANCEL, source='DJANGO_APP')
            print( 'ENQUEUED:', resp )
            # poll-id response
            data = { 'poll-id': resp }
