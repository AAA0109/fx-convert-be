...
class Command(TaskDefaultArgumentsMixin, BaseCommand):
    help = "Django command to cache CorPay Settlement Account"
    ...
    def handle(self, *args, **options):
        try:
            ...
            for company in companies:
                logging.info(f"Running to cache CorPay Settlement Account for (ID={company.pk}).")

                service = CorPaySettlementAccountCacheService()
                service.execute(company.pk)

                logging.info(f"Finished cache CorPay Settlement Account for (ID={company.pk}).")
