...
def get_data(
    ...
    qs = asset.fxforward_set.filter(**_filter_params)
    if not qs.exists():
        return pd.DataFrame()
...
if __name__ == '__main__':
    ...
    for i in pair_ids:
        try:
            ...
            df_out[["rate"]].plot(marker='.', ms=5)
            plt.title(f'asset: {asset.name}, tenor: {tenor}')

            dir_name = get_dir_to_store_plot('fxforward', start_date, end_date)
