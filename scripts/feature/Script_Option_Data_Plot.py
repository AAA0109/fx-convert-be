...
def get_data(
    ...
    _filter_params["call_put"] = call_put
    _filter_params["delta"] = delta

    qs = asset.option_set.filter(**_filter_params)
    if not qs.exists():
        return pd.DataFrame()
    df = pd.DataFrame(list(qs.all().values()))
    df["delta"] = df["delta"].astype(int).astype(str)
    return df
...
if __name__ == '__main__':
    ...
    for i in pair_ids:
        try:
            ...
            df_out[["mid_vol"]].plot(marker='.', ms=5)
            plt.title(f'asset: {asset.name}, tenor: {tenor}, strategy: {call_put}, offset: {delta}')

            dir_name = get_dir_to_store_plot('fxoption', start_date, end_date)
