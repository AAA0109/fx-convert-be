...
from hdlib.Core.Currency import EUR
...
def plot_risk_cones(ax, risk_cones):
    ...
    for it, data in enumerate(uppers):
        ax.plot(dates, initial_value + data, linestyle="--")  # label=f"Upper cone {it}",
    for it, data in enumerate(lowers):
        ax.plot(dates, initial_value + data, linestyle="--")  # label=f"Lower cone {it}",
...
def plot_values_and_risk_cones(ax, snapshots, risk_cones_by_date):
    ...
    ax.plot(dates, values, label="Account value", color="black")
...
def run():
    ...
    scenario.create_company(company_name="Test1", currency="USD")
    scenario.create_account(company_name="Test1", account_name="A")

    # Add a recurring cashflow.
    cf1 = RecurringCashFlowHDL(currency=EUR, amount=10000,
    ...
    cf2 = RecurringCashFlowHDL(currency=EUR, amount=-15000,
    ...
    for chunks in range(2):
        ...
        try:
            risk_cones = CashFlowRiskService().get_cashflow_risk_cones(domestic="USD",
